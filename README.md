# ffmpeg-stream-failover-system

Switch from primary to backup streams when health checks fail.
...
## Setup
```bash
cd ffmpeg-stream-failover-system
...
### Example
```bash
docker compose run --rm ffmpeg-tool python app.py --dry-run --primary rtmp://primary/live --backup rtmp://backup/live
```
